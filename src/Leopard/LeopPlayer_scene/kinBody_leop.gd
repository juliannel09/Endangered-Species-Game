...
func _ready():
	...
	get_node(".").move_to(Vector2(390, newBack.get_node("ground/ground_mech").get_pos().y - 132))
...
func _fixed_process(delta):
	if Globals.get("gameRun_leop")==true:
		...
		if((newBack.get_node("ground/ground_mech").get_pos().y - 132)<=(get_node(".").get_pos().y)) \
			&& (newBack.get_node("ground/ground_mech").get_pos().y - 130)>=(get_node(".").get_pos().y):
			print("can jump")
			jumpTimer = 0
