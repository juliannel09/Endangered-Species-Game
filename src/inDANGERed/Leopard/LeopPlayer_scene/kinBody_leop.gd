...
var ground = preload("res://Leopard/BG_leop_scene/BGleop_root.tscn")
#var net = preload("res://Leopard/Obstacles/Net_Node.tscn")
...
#				Globals.set("gameLevel", false)
			...
			elif Input.is_action_pressed("ui_jump") and Globals.get("canJump") == true:
				kinSpeed.y = JUMP
				motion = kinSpeed*delta
			else:
