...
func _ready():
	...
	get_node("Gillnet(2)_Area2D").connect("body_enter", self, "_test_collision")
	var x = 1940
	var y = 40
	get_node("Gillnet(2)_Area2D").set_pos(Vector2(x, y))

func _fixed_process(delta):
	if Globals.has("gameRun_leop"):
		get_node("Gillnet(2)_Area2D").set_pos(Vector2(get_node("Gillnet(2)_Area2D").get_pos().x - 18, get_node("Gillnet(2)_Area2D").get_pos().y))
