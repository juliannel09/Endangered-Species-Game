...
func _ready():
	...
	get_node("Fish_1_Area2D").connect("body_enter", self, "_test_collision")
	var x = 1940
	var y = 500
	get_node("Fish_1_Area2D").set_pos(Vector2(x, y))

func _fixed_process(delta):
	if Globals.has("gameRun_vaq"):
		get_node("Fish_1_Area2D").set_pos(Vector2(get_node("Fish_1_Area2D").get_pos().x - 7.5, get_node("Fish_1_Area2D").get_pos().y))
